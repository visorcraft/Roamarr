<script lang="ts">
	...
	function paymentStatusClass(statusValue: string | null | undefined) {
		return statusValue === 'fully_paid' || statusValue === 'deposit_paid'
			? 'trip-modern-segment-status-paid'
			: 'trip-modern-segment-status-confirmed';
	}
	...
	function segmentDetailRows(segment: SegmentRow): DetailRow[] {
		...
		if (local && segment.startAt) {
			rows.push({
				icon: 'calendar',
				label: 'Date & time',
				value: `${formatDate(local.toISODate()!)} · ${formatTime(segment.startAt, segment.startTz ?? 'UTC')}`
			});
		} else {
			rows.push({ icon: 'calendar', label: 'Date & time', value: 'Unscheduled' });
		}
...
<div class="trip-detail trip-modern">
	<!-- Hero: intentionally preserved from the existing trip detail page. -->
								...
								{#if isEditor}
									...
									{#if data.owner === true}
										...
										{#if data.publicShareUrl}
											<CopyButton text={data.publicShareUrl} class="app-user-menu-item app-user-menu-button w-full" label="Copy public link" icon="copy" />
										{/if}
									{/if}
								{/if}
							</div>
	...
	{#if data.nextCity && data.tileConfig}
		<GlobeModal bind:open={globeOpen} lat={data.nextCity.lat} lng={data.nextCity.lng} cityName={data.nextCity.cityName} />
	{/if}

	<div class="trip-modern-workspace">
		<div class="trip-modern-tabs-wrap">
			<nav class="trip-modern-tabs" aria-label="Trip sections">
				{#each tripTabs as tab (tab.id)}
					<button
						type="button"
						aria-current={activeTab === tab.id ? 'page' : undefined}
						class="trip-modern-tab {activeTab === tab.id ? 'trip-modern-tab-active' : ''}"
						onclick={() => selectTripTab(tab.id)}
					>
						<Icon name={tab.icon} class="h-4 w-4" />
						<span>{tab.label}</span>
						{#if tab.count != null && tab.count > 0}
							<span class="trip-modern-tab-count">{tab.count}</span>
						{/if}
					</button>
				{/each}
			</nav>
		...
		{#if activeTab === 'itinerary'}
			<div class="trip-modern-toolbar">
				<div class="trip-modern-control" aria-label="Trip date range">
				...
				<label class="trip-modern-search">
					<Icon name="search" class="h-4.5 w-4.5" />
					<span class="sr-only">Search itinerary</span>
					<input type="search" placeholder="Search itinerary…" bind:value={segmentQuery} />
				</label>

				<div class="trip-modern-toolbar-actions">
					<button type="button" class="trip-modern-control" onclick={() => (showTypeFilters = !showTypeFilters)}>
						<Icon name="settings" class="h-4 w-4" />
						<span>Filters</span>
					</button>
					<button type="button" class="trip-modern-control" onclick={() => (sortAscending = !sortAscending)}>
						<span>{sortAscending ? '↑↓' : '↓↑'}</span>
						<span>Sort</span>
					</button>
					<div class="trip-modern-view-toggle" aria-label="Itinerary view">
						{#each VIEW_MODES as mode}
							<button
								type="button"
								class="trip-modern-view-button {viewMode === mode ? 'trip-modern-view-button-active' : ''}"
								onclick={() => (viewMode = mode)}
							>
								<Icon name={mode === 'timeline' ? 'trips' : mode === 'list' ? 'document' : 'card'} class="h-4 w-4" />
								<span>{viewModeLabel(mode)}</span>
							</button>
						{/each}
					</div>
					{#if isEditor}
						<div class="trip-modern-add-menu">
							<a href={`/trips/${trip.id}/segments/new`} class="trip-modern-add-button">
								<Icon name="plus" class="h-4 w-4" />
								<span>Add segment</span>
							</a>
							<a href={`/trips/${trip.id}/segments/new`} class="trip-modern-add-caret" aria-label="Add segment options">
								<Icon name="chevron-down" class="h-4 w-4" />
							</a>
						</div>
					{/if}
			...
			{#if showTypeFilters && typeCounts.length}
				<div class="trip-modern-filter-row" aria-label="Filter plans by type">
					{#each typeCounts as t (t.type)}
						{@const active = selectedTypes.has(t.type)}
						<button type="button" class="trip-modern-filter-chip {active ? 'trip-modern-filter-chip-active' : ''}" onclick={() => toggleType(t.type)}>
							<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4">{@html SEG[t.type].icon}</svg>
							<span>{SEG[t.type].label}</span>
							<span class="font-mono">{t.count}</span>
						</button>
					{/each}
					{#if selectedTypes.size}
						<button type="button" class="trip-modern-filter-chip" onclick={() => (selectedTypes = new Set())}>Clear filters</button>
					{/if}
				</div>
			{/if}

			{#if isEditor}
				<form method="POST" action="?/moveSegmentDate" class="hidden" bind:this={moveSegmentDateForm} use:enhance={preserveScrollOnMove}>
				...
				<main class="trip-modern-main trip-modern-main-{viewMode}" id="trip-panel-itinerary">
					{#if ownerTrip?.notes}
						<section class="trip-modern-panel">
							<div class="trip-modern-panel-head">
								<h2 class="trip-modern-panel-title">Overview</h2>
							</div>
					...
					{#if dayGroups.length}
						{#each dayGroups as group (group.key)}
							<section
								class="trip-modern-day-card {dragOverDate === group.key ? 'trip-modern-drop-target' : ''}"
							...
							>
								<header class="trip-modern-day-head">
									<div class="trip-modern-day-title">
										<Icon name="calendar" class="h-5 w-5" />
										<div>
											<h2 class="trip-modern-day-date">{group.label}</h2>
											{#if groupWeekday(group.key)}<p class="trip-modern-day-weekday">{groupWeekday(group.key)}</p>{/if}
										</div>
									</div>
									<div class="flex items-center gap-3">
										<span class="trip-modern-day-count">{group.segments.length} segment{group.segments.length === 1 ? '' : 's'}</span>
										{#if isEditor}
											<a href={`/trips/${trip.id}/segments/new`} class="trip-modern-day-plus" aria-label="Add segment"><Icon name="plus" class="h-4 w-4" /></a>
										{/if}
									</div>
								</header>
								<div class="trip-modern-day-body">
									{#each group.segments as s, i (s.id ?? `${group.key}-${i}`)}
										...
										{@const paymentLabel = paymentStatusLabel(s.paymentStatus)}
										...
										{:else}
											<article
												class="trip-modern-segment {segmentTypeClass(s.type)} {selectedSegment?.id != null && selectedSegment.id === s.id ? 'trip-modern-segment-active' : ''} {draggingSegmentId === s.id ? 'trip-modern-segment-dragging' : ''}"
												role="button"
												tabindex="0"
												onclick={(e) => handleSegmentCardClick(s, e)}
												onkeydown={(e) => handleSegmentKeydown(s, e)}
											...
											>
												...
												<span class="trip-modern-type-node" aria-hidden="true">
													<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-5 w-5">{@html SEG[s.type as keyof typeof SEG]?.icon ?? ''}</svg>
												</span>
												<div class="trip-modern-segment-body">
													<div class="trip-modern-segment-head">
														<h3 class="trip-modern-segment-title">{s.title}</h3>
														{#if s.status}<span class="trip-modern-segment-status {segmentStatusClass(s.status)}">{segmentStatusLabel(s.status)}</span>{/if}
														{#if paymentLabel}<span class="trip-modern-segment-status {paymentStatusClass(s.paymentStatus)}">{paymentLabel}</span>{/if}
													</div>
													<p class="trip-modern-segment-subtitle">{segmentSubtitle(s)}</p>
													{#if segmentMetaItems(s).length}
														<div class="trip-modern-segment-meta">
															{#each segmentMetaItems(s) as item}
																<span class="trip-modern-meta-pill">• {item}</span>
															{/each}
														</div>
													{/if}
												</div>
												<Icon name="chevron-down" class="trip-modern-chevron h-5 w-5 -rotate-90" />
											</article>
										{/if}
									{/each}
								</div>
						...
						{/each}
					{:else}
						<div class="trip-modern-empty">No itinerary segments match your filters.</div>
					{/if}
				</main>

				<aside class="trip-modern-side-panel" aria-label="Selected segment details">
					{#if selectedSegment}
						<header class="trip-modern-selected-head">
							<h2 class="trip-modern-selected-title">Selected segment</h2>
							<button type="button" class="icon-button icon-button-sm" aria-label="Clear selected segment" onclick={() => (focusedSegmentId = null)}>
						...
						<div class="trip-modern-selected-summary {segmentTypeClass(selectedSegment.type)}">
							...
							<div class="min-w-0">
								<h3 class="trip-modern-selected-name">{selectedSegment.title}</h3>
								<div class="mt-2 flex flex-wrap gap-1.5">
									{#if selectedSegment.status}<span class="badge badge-compact {segmentStatusClass(selectedSegment.status)}">{segmentStatusLabel(selectedSegment.status)}</span>{/if}
									{#if selectedSegment.paymentStatus}<span class="badge badge-compact badge-green">{paymentStatusLabel(selectedSegment.paymentStatus)}</span>{/if}
								</div>
								<p class="trip-modern-panel-muted mt-2 text-sm">{segmentSubtitle(selectedSegment)}</p>
							</div>
						</div>

						<nav class="trip-modern-detail-tabs" aria-label="Selected segment sections">
							{#each SEGMENT_PANEL_TABS as tab (tab.id)}
								<button type="button" class="trip-modern-detail-tab {segmentPanelTab === tab.id ? 'trip-modern-detail-tab-active' : ''}" onclick={() => (segmentPanelTab = tab.id)}>
									{tab.label}{#if tab.id === 'travelers' && selectedAttendees.length} ({selectedAttendees.length}){/if}
								</button>
							{/each}
						</nav>

						{#if segmentPanelTab === 'details'}
							<div class="trip-modern-detail-list">
								{#each selectedDetailRows as row (row.label)}
									<div class="trip-modern-detail-row">
										<Icon name={row.icon} class="trip-modern-detail-icon h-4 w-4" />
										<span class="trip-modern-detail-label">{row.label}</span>
										<span class="trip-modern-detail-value">{row.value}</span>
									</div>
								{/each}
							</div>
						{:else if segmentPanelTab === 'travelers'}
							<div class="trip-modern-detail-list">
								{#if selectedAttendees.length}
									{#each selectedAttendees as a (a.id)}
										<div class="trip-modern-list-row">
											<span>{a.name}</span>
											<span class="badge badge-compact {a.status === 'going' ? 'badge-green' : a.status === 'maybe' ? 'badge-amber' : 'badge-slate'} capitalize">{a.status.replace('_', ' ')}</span>
										</div>
									{/each}
								{:else}
									<p class="trip-modern-empty">No travelers assigned to this segment.</p>
								{/if}
							</div>
						{:else if segmentPanelTab === 'notes'}
							<div class="trip-modern-detail-list">
								{#if detailsEntries(selectedDetailsPayload).length}
									{#each detailsEntries(selectedDetailsPayload) as [key, value] (key)}
										<div class="trip-modern-list-row">
											<span class="capitalize">{key.replaceAll('_', ' ')}</span>
											<span class="trip-modern-panel-muted">{renderJsonValue(value)}</span>
										</div>
									{/each}
								{:else}
									<p class="trip-modern-empty">No extra notes for this segment.</p>
								{/if}
							</div>
						{:else}
							<div class="trip-modern-detail-list">
						...
						{#if isEditor && selectedSegment.id}
							<div class="trip-modern-selected-actions">
								<button type="button" class="btn btn-primary" onclick={() => (editingId = selectedSegment.id ?? null)}>
								...
								<form method="POST" action={`/trips/${trip.id}/segments?/delete`}>
									<input type="hidden" name="segmentId" value={selectedSegment.id} />
									<button class="btn btn-danger" onclick={(e) => { if (!confirm('Delete this segment?')) e.preventDefault(); }}>
										<Icon name="close" class="h-4 w-4" />
										Delete
									</button>
								</form>
					...
					{:else}
						<div class="trip-modern-selected-empty">
							<div>
								<Icon name="trips" class="mx-auto mb-3 h-8 w-8" />
								<p>Select a segment to see its details.</p>
							</div>
						</div>
					{/if}
				</aside>
		...
		{:else if activeTab === 'prep'}
			<section class="trip-modern-panel">
				<div class="trip-modern-panel-head">
					<h2 class="trip-modern-panel-title">Prep</h2>
					<span class="badge badge-slate badge-compact">{prepItemCount} item{prepItemCount === 1 ? '' : 's'}</span>
				</div>
				{#if data.checklist?.items?.length}
					<div class="trip-modern-list">
						{#each data.checklist.items as item (item.id)}
							<div class="trip-modern-list-row">
								<form method="POST" action="?/toggleChecklistItem" class="flex min-w-0 items-center gap-3">
									<input type="hidden" name="itemId" value={item.id} />
									<button class="check-toggle" type="submit">
										<span class="check-marker {item.packed ? 'check-marker-active' : ''}"><Icon name="check" class="h-3.5 w-3.5" /></span>
										<span class="check-label {item.packed ? 'check-label-done' : ''}">{item.text}</span>
									</button>
								</form>
								{#if item.assignedToName}<span class="badge badge-slate badge-compact">{item.assignedToName}</span>{/if}
							</div>
						{/each}
					</div>
				{:else}
					<p class="trip-modern-empty">No prep items yet.</p>
				{/if}
				{#if isEditor}
					<form method="POST" action="?/addChecklistItem" class="trip-modern-form-inline">
						<input name="text" class="input flex-1 text-sm" placeholder="Add checklist item" required />
		...
		{:else if activeTab === 'money'}
			<section class="trip-modern-panel">
				<div class="trip-modern-panel-head">
					<h2 class="trip-modern-panel-title">Budget & expenses</h2>
				...
				{:else}
					<p class="trip-modern-empty mt-5">No expenses recorded yet.</p>
				{/if}
			</section>
