...
export const GET: RequestHandler = async ({ url, locals, getClientAddress }) => {
	...
	const { page, limit: pageLimit } = parseTableParams(url);
	...
	const filtered = q
		...
		: all;
	const trips = filtered.slice((page - 1) * pageLimit, page * pageLimit).map((t) => ({
		id: t.id,
		label: t.name,
	...
	return json({ trips, total: filtered.length });
};
