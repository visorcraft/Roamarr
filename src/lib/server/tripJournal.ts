...
export async function addJournalEntry(event: RequestEvent) {
	...
	const v = new Validator();
	const entryDate = v.requiredDate(f.get('entryDate'), 'entryDate');
	const title = v.requiredString(f.get('title'), 'title', { max: 200 });
	const body = v.requiredString(f.get('body'), 'body', { max: 10000 });
	if (!v.ok()) {
		return fail(400, { error: v.failMessage(), errors: v.errors });
	}
	createJournalEntry(u.id, tripId, {
		entryDate: entryDate!,
		title: title!,
		body: body!
	});
...
export async function updateJournalEntry(event: RequestEvent) {
	...
	if (!Number.isFinite(tripId)) throw error(404, 'Not found');
	const f = await event.request.formData();
	const entryId = Number(f.get('entryId'));
	if (!Number.isFinite(entryId) || entryId <= 0) {
		return fail(400, { error: 'Invalid journal entry' });
	}
	const v = new Validator();
	const entryDate = v.requiredDate(f.get('entryDate'), 'entryDate');
	const title = v.requiredString(f.get('title'), 'title', { max: 200 });
	...
	modifyJournalEntry(u.id, entryId, {
		entryDate: entryDate!,
		title: title!,
		body: body!
	});
...
export async function deleteJournalEntry(event: RequestEvent) {
	...
	removeJournalEntry(u.id, entryId);
	throw redirect(303, `/trips/${tripId}`);
}
