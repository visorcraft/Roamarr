<script lang="ts">
	...
	let tripActionError = $state<string | null>(null);
	let tripActionMessage = $state<string | null>(null);
	...
	async function submitTripActionForm(event: SubmitEvent, after?: () => void) {
		event.preventDefault();
	...
	function handleReminderSubmit(event: SubmitEvent) {
		void submitTripActionForm(event, () => {
			segmentPanelTab = 'reminders';
			tripActionMessage = 'Reminder saved.';
		});
	}

	function handleDuplicateSubmit(event: SubmitEvent) {
		void submitTripActionForm(event, () => {
			tripActionMessage = 'Segment duplicated.';
		});
	}
...
<div class="trip-detail trip-modern">
	...
	{#if tripActionError}<p class="notice notice-error trip-detail-body mt-3">{tripActionError}</p>{/if}
	{#if tripActionMessage}<p class="notice notice-success trip-detail-body mt-3">{tripActionMessage}</p>{/if}
	...
	<div class="trip-modern-workspace">
		...
		{:else if activeTab === 'money'}
			<section class="trip-modern-panel"><div class="trip-modern-panel-head"><h2 class="trip-modern-panel-title">Budget & expenses</h2>{#if data.expenses?.length}<button type="button" class="btn btn-secondary btn-sm" onclick={exportExpensesCsv}>Export CSV</button>{/if}</div><div class="grid gap-3 md:grid-cols-3"><div class="metric-card"><span class="metric-label">Expenses</span><p class="metric-value">{formatCents(data.stats?.totalExpenses ?? 0, data.stats?.totalExpensesCurrency ?? baseCurrency)}</p></div><div class="metric-card"><span class="metric-label">Budget cap</span><p class="metric-value">{formatCents(data.stats?.budgetCap ?? 0, baseCurrency)}</p></div><div class="metric-card"><span class="metric-label">Paid segments</span><p class="metric-value">{data.stats?.paidSegments ?? 0}/{data.stats?.totalSegments ?? segmentList.length}</p></div></div>{#if categorySpending.length}<div class="mt-5 trip-modern-list">{#each categorySpending as c (c.category)}<div class="trip-modern-list-row"><span>{c.label}</span><div class="min-w-0 flex flex-1 items-center gap-3"><div class="progress-track"><div class="progress-fill" style="width: {Math.round((c.amount / maxCategorySpend) * 100)}%;"></div></div><span class="font-mono text-sm">{formatCents(c.amount, baseCurrency)}</span></div></div>{/each}</div>{/if}{#if data.expenses?.length}<div class="mt-5 trip-modern-list">{#each data.expenses as e (e.id)}<div class="trip-modern-list-row"><div><strong>{e.description}</strong><p class="trip-modern-panel-muted text-sm capitalize">{e.category} · paid by {settlementName(e.paidBy)}</p></div><span class="font-mono">{e.currency} {(e.amount / 100).toFixed(2)}</span></div>{/each}</div>{:else}<p class="trip-modern-empty mt-5">No expenses recorded yet.</p>{/if}</section>
