...
import { tripMedications, tripCompanions, trips, users } from './db/mongrelSchema';
...
beforeEach(() => {
	const kit = getKit();
	kit.deleteFrom(tripMedications).executeSync();
...
test('addMedication creates a schedule with companion name', () => {
	...
	expect(kit.selectFrom(tripMedications).where(eq(tripMedications.id, BigInt(med.id))).executeSync()[0]).toBeTruthy();
});
...
test('deleteMedication removes the schedule', () => {
	...
	expect(kit.selectFrom(tripMedications).where(eq(tripMedications.id, BigInt(med.id))).executeSync()[0]).toBeUndefined();
});
